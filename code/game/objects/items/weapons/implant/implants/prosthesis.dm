...
/obj/item/weapon/implant/prosthesis_inhibition/get_data()
	var/dat = {"
...
/obj/item/weapon/implant/prosthesis_inhibition/on_install(mob/living/carbon/human/source)
	if(malfunction)
		return
	...
	for(var/obj/item/organ/external/E in source.organs)
		if(istype(E, /obj/item/organ/external/robotic))
			...
			R.deactivate()
			source << SPAN_WARNING("Your [E] automaticly deactivated and locked!")
		if(istype(E.module, /obj/item/organ_module/active))
			var/obj/item/organ_module/active/M = E.module
			M.deactivate(source, E)
			source << SPAN_WARNING("[M] in your [E] automaticly deactivated and locked!")